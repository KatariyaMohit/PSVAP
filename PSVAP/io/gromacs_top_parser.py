"""
io/gromacs_top_parser.py
------------------------
Parser for GROMACS topology files (.top).

GROMACS .top files are text files that define:
  - Atom types and parameters
  - Atomic masses, charges, van der Waals parameters
  - Bond types and force constants
  - Angle and dihedral definitions
  - Molecules and their atom counts

This parser extracts atom topology and connectivity information.

Rule compliance
---------------
  Rule 4  : One parser — GROMACS .top format only.
...
class GromacsTopFormatError(ValueError):
    """Raised when a GROMACS .top file cannot be parsed."""


class GromacsTopParser(BaseParser):
    """
    Parses GROMACS topology files (.top).

    Since .top files define molecular structure without explicit coordinates,
    we create atoms with zero positions. This is a topology-only file.
    For actual coordinates, pair with a .gro or .xtc file.
    """
    ...
    def parse(self, path: Path) -> tuple[list[Atom], list[np.ndarray], SystemMetadata]:
        ...
        -------
        atoms : list[Atom]
            Atoms defined in the topology (with zero coordinates)
        trajectory : list[np.ndarray]
            Single frame at origin (shape: (N, 3) of zeros)
        ...
        try:
            ...
            raise GromacsTopFormatError(f"Cannot read .top file '{path.name}': {exc}") from exc

        atoms, bonds = self._parse_content(content, path)

        if not atoms:
            raise GromacsTopFormatError(f"No atoms found in '{path.name}'")
    ...
    def _parse_content(
        ...
        """
        Parse GROMACS topology content.

        Extracts atoms section and bonds section.
        """
        atoms: list[Atom] = []
    ...
    def _bonds_to_pyvista_format(bonds_list: list[tuple[int, int]]) -> np.ndarray | None:
        ...
        Format: [2, i, j, 2, i2, j2, ...] where 2 is the line segment count.
        """
        if not bonds_list:
            return None
